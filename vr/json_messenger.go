...
func (s *JsonMessenger) SendPrepareOK(uri string, from int64, to int64, backupView int64, backupOp int64,
	...
	return send(uri, PrepareOKEndpoint, PrepareOKMessage{from, to, backupView, backupOp, backupReplica})
}
...
func (s *JsonMessenger) ReceivePrepareOK() (from int64, to int64, backupView int64, backupOp int64, backupReplica int64, err error) {
	msg, ok := <-s.prepareOKMessages
	if !ok {
		err = errors.New("Error: no more incoming entries")
		return
	}
	return msg.from, msg.to, msg.backupView, msg.backupOp, msg.backupReplica, err
}
